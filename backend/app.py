...
HUGGING_FACE_API_TOKEN = os.getenv('HUGGING_FACE_API_TOKEN', 'hf_qcZLjqECToXfFpqyMfIgpVrXQDnntiKoNxp')
...
def init_db():
    ...
    # User feedback and preferences table
    cursor.execute('''
...
def fallback_analysis(text):
    """Enhanced fallback analysis when API is unavailable"""
...
def generate_personalized_recommendations(mood_score, sentiment_label, emotions, text, user_profile=None):
    """Enhanced recommendation engine that uses onboarding data"""
    recommendations = []
    ...
    # Use onboarding data for personalization
    if user_profile:
...
def analyze_mood_with_ai(text, user_id=1):
    """Enhanced mood analysis with personalization"""
...
def send_confirmation_email():
    ...
    try:
        ...
        # Validate that email credentials are loaded
...
def analyze_mood():
    """Enhanced mood analysis endpoint"""
    try:
        data = request.get_json()
...
def rate_recommendation():
    """Allow users to rate recommendation effectiveness"""
    try:
        data = request.get_json()
        user_id = data.get('user_id', 1)
...
def health_check():
    ...
    return jsonify({
        ...
        'features': ['ai_analysis', 'crisis_detection', 'personalized_recommendations', 'email_notifications']
    })
...
if __name__ == '__main__':
    ...
    print("📧 Email notifications configured" if SENDER_EMAIL and SENDER_PASSWORD else "⚠️ Email credentials missing - set SMTP_EMAIL and SMTP_PASSWORD in .env")
